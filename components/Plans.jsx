      ...
      <div className=' mt-20 grid grid-cols-1 max-md:max-w-sm md:grid-cols-3 md:max-xl:w-[800px] gap-y-16 xl:grid-cols-6'>
        {plans.map((plan,idx) =>
          <div className={`bg-white text-black flex flex-col border md:border-r-0 border-gray-200 relative p-5 basis-full ${idx === 2 || idx === 3 ? '-mt-8' : ''}`}>
            ...
            <Link href="/start"  className={`button sticky top-2 font-medium ${plan.buttonColor} text-white h-10 flex items-center justify-center rounded-md my-4 text-sm`}>{idx === 0 ? 'Start with Free' : 'Get ' + plan.name}</Link>
            <p className={`h-10 py-3 text-xs ${plan.textColor} font-bold`}>{idx != 0 ? `Everything in ${plans[idx-1].name}, plus:` : ''}</p>
            <ul>
            {plan.features.map(feature => <li key={feature} className={`${feature === "Free domain for one year" && activeButton === 'monthly' ? 'line-through font-bold' : feature === "Free domain for one year" ? 'font-bold' : ''} text-xs mb-4`}>{feature}</li>)}
            </ul>
            ...
            <ul>
              {plan.tools.map(tool => <li key={tool} className='text-xs mb-4'>{tool}</li>)}
            </ul>
        ...
        <div className='bg-white text-black flex flex-col border border-gray-200 relative p-5 basis-full'>
          ...
          <Link href="/start" className='button sticky top-2 font-medium bg-[#2c3338] text-white h-10 rounded-md my-4 text-sm flex items-center justify-center'>Get in touch</Link>
